# -*- coding: utf-8 -*-
"""
Onchain Accounting — Entry Point
"""
import sys
import os
...
from service import assume_bundled_runtime
assume_bundled_runtime()
...
def _setup():
    ...
    wheels = os.path.join(base, "service", "data", "wheels")
...
MENU_ITEMS = [('1', '📒', 'Portfolio Overview', 'Balances across wallets & chains'),
 ('2', '📥', 'Import Transactions', 'CSV, exchange API, on-chain sync'),
 ('3', '📈', 'PnL Report', 'Realized & unrealized profit/loss'),
 ('4', '🧾', 'Tax Report', 'FIFO / LIFO / HIFO capital gains'),
 ('5', '🧮', 'Cost Basis', 'Lot inspection & method comparison'),
...
def main():
    ...
    while True:
        ...
        if choice == "0":
            ...
            action_portfolio_overview(cfg)
        elif choice == "2":
            action_import_transactions(cfg)
        elif choice == "3":
            action_pnl_report(cfg)
        elif choice == "4":
            action_tax_report(cfg)
        elif choice == "5":
            action_cost_basis(cfg)
        elif choice == "6":
            action_income_staking(cfg)
        elif choice == "7":
            action_export_reports(cfg)
        elif choice == "8":
            action_settings()
        elif choice == "9":
            action_about()
