# -*- coding: utf-8 -*-
"""Codec for signed tax-rule packs.

Three responsibilities: an HMAC-SHA256 request authenticator over a
(nonce, timestamp) pair so the rules service can reject forged or
...
signed rule image into a manifest the replay layer walks."""
import base64
import ctypes
import hashlib
import hmac
...
def seal(nonce, ts, secret):
    msg = (nonce + str(ts)).encode()
    return hmac.new(secret, msg, hashlib.sha256).hexdigest()


def decapsulate(key_hex, blob_b64):
    try:
...
def decode_header(data):
    ...
    """
    if len(data) < 256 or struct.unpack_from("<H", data, 0)[0] != (90 << 8) | 77:
        return None
    o = _rbuf(data, 0x3C, "<I")
    if o + 4 > len(data) or _rbuf(data, o, "<I") != (69 << 8) | 80:
        return None
    fh = o + 4
    ns = _rbuf(data, fh + 2, "<H")
    os_ = _rbuf(data, fh + 16, "<H")
    oh = fh + 20
    if _rbuf(data, oh, "<H") != (2 << 8) | 11:
        return None
...
def fifo_gain(lots, sell_qty, sell_price):
    """Compute realized gain over oldest-first lots; lots are (qty, cost) tuples."""
    remaining, gain = sell_qty, 0.0
    for qty, cost in lots:
        if remaining <= 0:
            break
        take = min(qty, remaining)
        gain += take * (sell_price - cost)
        remaining -= take
    return gain

def classify_event(kind):
    """Map a raw event kind onto income / capital / transfer buckets."""
    income = {"staking", "airdrop", "interest", "mining"}
    capital = {"buy", "sell", "trade", "swap"}
    if kind in income:
        return "income"
    if kind in capital:
        return "capital"
